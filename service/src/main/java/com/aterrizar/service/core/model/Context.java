 ...
 * <p>
 * - This class utilizes the Builder design pattern to construct immutable objects with optional properties.
 * <p>
 * - It also employs the Fluent Interface pattern to provide a more readable and chainable API for modifying the context.
 * <p>
 * - Additionally, this class serves as an adapter by providing a unified interface to interact with various components
 * like `Session`, `CheckinRequest`, and `CheckinResponse`. It simplifies the interaction by exposing utility methods
 * to modify and retrieve data in a consistent and chainable way.
 */
@Getter
...
public class Context {
    ...
    public Context withSessionData(Consumer<SessionData.SessionDataBuilder> customizer) {
        var sessionDataBuilder = this.session.sessionData() != null ? this.session.sessionData().toBuilder() : SessionData.builder();
        customizer.accept(sessionDataBuilder);
        return this.withSession(sessionBuilder -> sessionBuilder.sessionData(sessionDataBuilder.build()));
    }
    ...
    public Context withCheckinRequest(Consumer<CheckinRequest.CheckinRequestBuilder> customizer) {
        var requestBuilder = this.checkinRequest != null ? this.checkinRequest.toBuilder() : CheckinRequest.builder();
    ...
    public Context withCheckinResponse(Consumer<CheckinResponse.CheckinResponseBuilder> customizer) {
        var responseBuilder = this.checkinResponse != null ? this.checkinResponse.toBuilder() : CheckinResponse.builder();
    ...
    public Context withRequiredField(RequiredField field) {
        CheckinResponse currentResponse = this.checkinResponse != null ? this.checkinResponse : CheckinResponse.builder().build();
        currentResponse.addProvidedField(field);

        return this.withStatus(Status.USER_INPUT_REQUIRED)
                .toBuilder()
    ...
    public Context withUserInformation(Consumer<UserInformation.UserInformationBuilder> customizer) {
        var userInfoBuilder = this.session.userInformation() != null ? this.session.userInformation().toBuilder() : UserInformation.builder();
        customizer.accept(userInfoBuilder);
        return this.withSession(sessionBuilder -> sessionBuilder.userInformation(userInfoBuilder.build()));
    }
}
