...
class PaymentValidationStepTest {
    ...
    void shouldRequestCardNumberWhen3dsIsSelectedButEmpty() {
        ...
        var context = MockContext.initializedMock(CountryCode.US)
                .withCheckinRequest(builder -> builder.providedFields(Map.of(
                        ...
                        // It's missing the CARD_NUMBER, which is required for 3DS
                )));

        var result = paymentValidationStep.onExecute(context);

        assertTrue(result.isTerminal());
        assertTrue(result.context().checkinResponse().providedFields().contains(RequiredField.CARD_NUMBER));
    }
    ...
    void shouldGenerateTokenAndSaveInSessionWhenDataIsComplete() {
        ...
        var context = MockContext.initializedMock(CountryCode.CA)
                .withCheckinRequest(builder -> builder.providedFields(Map.of(
