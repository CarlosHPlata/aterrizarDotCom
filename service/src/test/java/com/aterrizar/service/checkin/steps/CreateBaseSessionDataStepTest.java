...
class CreateBaseSessionDataStepTest {
    ...
    void shouldReturnSuccessWhenSessionRequestIsValid() {
        ...
        var context = InitContext.builder()
                ...
                .build();
        var flight =FlightData.builder()
                .flightNumber("abc")
                .build();

        when(flightGateway.getFlightData(sessionRequest.flights()))
        ...
        assertEquals(flight.flightNumber(), updatedContext.session().sessionData().flights().get(0).flightNumber());
    }
